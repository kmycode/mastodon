...
RSpec.describe ActivityPub::Activity::Like do
  ...
  describe '#perform when receive emoji reaction' do
    ...
    before do
      ...
      stub_request(:get, 'https://example.com/aaa').to_return(status: 200, body: original_emoji.to_json, headers: { 'Content-Type': 'application/activity+json' })
      stub_request(:get, 'https://example.com/invalid').to_return(status: 200, body: original_invalid_emoji.to_json, headers: { 'Content-Type': 'application/activity+json' })
    end
