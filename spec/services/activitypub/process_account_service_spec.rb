...
RSpec.describe ActivityPub::ProcessAccountService do
  ...
  def stub_webfinger!(software: '')
    ...
    stub_request(:get, 'https://example.com/.well-known/nodeinfo').to_return(body: "{\"software\":\"#{software}\"}", headers: { 'Content-Type': 'application/jrd+json' })
    stub_request(:get, 'https://foo.test/.well-known/nodeinfo').to_return(body: "{\"software\":\"#{software}\"}", headers: { 'Content-Type': 'application/jrd+json' })
  end
  ...
  describe 'about blocking new remote account' do
    ...
    context 'when is blocked' do
      ...
      context 'with has existing account' do
        before do
          Fabricate(:account, uri: 'https://foo.test', domain: 'foo.test', username: 'alice', note: 'old bio')
        end
      ...
      context 'with has existing suspended pending account' do
        before do
          Fabricate(:account, uri: 'https://foo.test', domain: 'foo.test', username: 'alice', note: 'old bio', suspended_at: 1.day.ago, remote_pending: true, suspension_origin: :local)
        end
      ...
      context 'with has existing suspended account' do
        before do
          Fabricate(:account, uri: 'https://foo.test', domain: 'foo.test', username: 'alice', note: 'old bio', suspended_at: 1.day.ago, suspension_origin: :local)
        end
  ...
  context 'with searchability' do
    ...
    before do
      stub_webfinger!(software: software)
      Fabricate(:instance_info, domain: 'foo.test', software: software)
      stub_request(:get, 'https://foo.test/followers').to_return(body: '[]')
    end
    ...
    context 'when private' do
      let(:searchable_by) { 'https://foo.test/followers' }
  ...
  context 'with subscription policy' do
    ...
    context 'when private' do
      let(:subscribable_by) { 'https://foo.test/followers' }
  ...
  context 'when account is using note contains ng words' do
    ...
    let!(:account) { Fabricate(:account, username: 'alice', domain: 'foo.test') }
  ...
  context 'when discovering many subdomains in a short timeframe' do
    ...
    before do
      ...
      8.times do |i|
        ...
        stub_request(:get, "https://#{domain}/.well-known/nodeinfo").to_return(body: '{}')
      end
    end
