...
class Admin::ModerationAction < Admin::BaseAction
  ...
  def mark_statuses_as_cw!
    representative_account = Account.representative

    statuses.find_each do |status|
      next if status.discarded? || status.spoiler_text.present?
      ...
      if target_account.local?
        UpdateStatusService.new.call(status, representative_account.id, sensitive: true, spoiler_text: 'CW', text: status_text, bypass_validation: true)
      else
        status.update(sensitive: true, spoiler_text: 'CW', text: status_text)
      end
