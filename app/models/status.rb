...
class Status < ApplicationRecord
  ...
  def update_conversation
    return if reply?
    return if reply? && !thread.nil? && (!limited_visibility? || none_limited? || reply_limited?)

    conversation.update!(parent_status: self, parent_account: account, ancestor_status: self) if conversation && conversation.parent_status.nil?
  end
