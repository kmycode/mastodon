...
class About extends PureComponent {
  ...
  render () {
    ...
    const fedibirdCapabilities = server.item?.fedibird_capabilities || [];   // thinking about isLoading is true
    ...
    const isFullTextSearch = server.item?.search?.enabled;
    const isAcceptLegacyQuotes = server.item?.quotes?.auto_accept_legacy_quotes;
        ...
        <div className='scrollable about' id={getColumnSkipLinkId(1)}>
          ...
          <div className='about__meta'>
            ...
            <div className='about__meta__column'>
              ...
              {isLoading ? <Skeleton width='10ch' /> : <a className='about__mail' href={emailLink}>{server.item?.contact?.email}</a>}
            </div>
          </div>
                    ...
                    {domainBlocks.items.map(block => (
                      <div className='about__domain-blocks__domain' key={block.domain}>
                        <div className='about__domain-blocks__domain__header'>
                          ...
                          <span className='about__domain-blocks__domain__type' title={intl.formatMessage(severityMessages[block.severity].explanation)}>{intl.formatMessage(severityMessages[block.severity_ex || block.severity].title)}</span>
                        </div>
