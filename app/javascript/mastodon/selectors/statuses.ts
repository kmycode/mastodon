...
export const selectStatusConditions = createAppSelector(
  ...
  (status, userId, quotedAccountId, isLocal, account) => {
    const isPublic =
      !!status &&
    ...
    const isMineAndPrivate =
      userId === status?.account && status.visibility_ex === 'private';
    ...
    return {
      ...
      isNotDirect: !!status && status.visibility_ex !== 'direct',
      isPrivateReblog:
        userId === status?.account && status.visibility_ex === 'private',
      ...
      isStatusReferenceAvailableServer:
        !!account?.server_features.status_reference,
    };
  },
);
