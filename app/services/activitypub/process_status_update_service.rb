...
class ActivityPub::ProcessStatusUpdateService < BaseService
  ...
  def call(status, activity_json, object_json, request_id: nil)
    ...
    @json                      = object_json
    @status_parser             = ActivityPub::Parser::StatusParser.new(@json, account: status.account, followers_collection: status.account.followers_url, following_collection: status.account.following_url, actor_uri: ActivityPub::TagManager.instance.uri_for(status.account))
